...
<div class="modal fade" id="word_update_modal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
		...
		<div class="modal-content">
			...
			<div class="modal-body">
				<div class="thumbnail text-center">
					<div class="row">
						<img id="word_img" style="width:300px; height: 200px;" src="<?=$word->img?>" alt="" />
						...
						method="post" accept-charset="utf-8" style="display: none;" >
							...
							<input id="word_img_text" type="text" name="img" value="<?=$word->img?>"/>
						</form>
					</div>
					<div class="row">
						<div style="display:none">
							...
							<audio id="word_sound_tag" src="<?=$word->sound?>"></audio>
						</div>
						<div class="col-md-2 col-md-offset-2">
							<label for="">Sound :</label>
						</div>
						<div class="col-md-4">
							<input id="word_sound_text" type="text" class="form-control" name="sound" value="<?=$word->sound?>" readonly="readonly"/>
						</div>
						<div class="col-md-1">
					...
					<div class="row">
						<div class="form-group">
							...
							<div class="col-md-4">
								<input id="word_create_word" name="word" type="text" class="form-control" placeholder="(Required)" value="<?=$word->word?>"/>
							</div>
						</div>
					</div>
					<div class="row">
						<div class="form-group">
							...
							<div class="col-md-4">
								<textarea class="form-control" name="mean" id="word_create_mean" cols="15" rows="5"><?=$word->mean?></textarea>
							</div>
						</div>
					</div>
					<div class="row">
						<div class="form-group">
							...
							<div class="col-md-4">
								<textarea class="form-control" name="example" id="word_create_example" cols="15" rows="5"><?=$word->example?></textarea>
							</div>
						</div>
					</div>
				</div>
			...
			<div class="modal-footer">
				<button type="button" class="btn btn-default" data-dismiss="modal" onclick="wordCreateCancel()">
					...
					<button type="button" class="btn btn-danger" onclick="deleteWord(<?=$word->wid?>)">Delete</button>
				<button type="button" class="btn btn-primary" onclick="wordCreateSave()">
					Save
				</button>

				<form id="word_create_form" action="<?echo ROOT_URL?>/library/update_word_proc"				method="post" accept-charset="utf-8" style="display: none;"  enctype="multipart/form-data">
					<input type="text" id="wid_input" name="wid" value="<?=$word->wid?>"/>
				</form>
			</div>
		</div>
	</div>
...
<script type="text/javascript" charset="utf-8">
	...
	function deleteWord(wid) {
		var response = reqProc('/library/deleteWord/' + wid);
		if (response.status != 'error') {
			alert('deleted');
			$('#word_update_modal').modal('hide');
			return;
		}
		
		alert(response.msg);
	}
	$('#word_update_modal').on('hidden.bs.modal', function(e){
		$(this).remove();
	});
	...
	*/
	...
	$('#word_img_form').ajaxForm({
		...
		success : function(responseText, statusText) {
			$('#word_img').attr('src', '/wordForYou/uploads/tmp/' + responseText);
	...
	//submit이후의 처리
		success : function(responseText, statusText) {
			...
			$('#word_sound_tag').attr('src','/wordForYou/uploads/tmp/' + responseText);
		},
	//ajax error
		error : function(responseText, statusText) {
			alert(responseText['responseText']);
